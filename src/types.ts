...
export interface PinMethods {
  pin: (entry: HistoryEntry) => void
  moveUp: (entry: HistoryEntry) => void
  moveDown: (entry: HistoryEntry) => void
  unpin: (entry: HistoryEntry) => void
  unpinAll: () => void
  getAllowedMovements: (entry: HistoryEntry) => PinnedMovement[]
}

export interface RemoveMethods {
  removeEntry: (entry: HistoryEntry) => Promise<void>
